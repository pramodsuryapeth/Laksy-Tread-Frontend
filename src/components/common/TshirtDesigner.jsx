...
// ─── T‑shirt background (replace with your image) ─────────────────────────────
const TshirtBackground = () => (
  <svg viewBox="0 0 200 200" className="absolute inset-0 w-full h-full z-0" xmlns="http://www.w3.org/2000/svg">
    <path d="M40 20 L60 20 L60 0 L140 0 L140 20 L160 20 L180 60 L180 180 L20 180 L20 60 Z"
      fill="#f3f4f6" stroke="#d1d5db" strokeWidth="2" />
  </svg>
);

// ─── Small reusable UI atoms (unchanged, just kept for completeness) ────────
const SectionTitle = ({ children }) => (
  <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">
    {children}
  </h3>
...
// ─── RIGHT PANEL content (unchanged, still has onBuyNow) ─────────────────────
const RightPanel = ({ ctx, onDownload, onBuyNow }) => {
  const jsonRef = useRef(null);
  const [openSection, setOpenSection] = useState('history');
...
// ─── MAIN COMPONENT (responsive, t‑shirt background, smaller canvas) ────────
const TshirtDesigner = ({ product, variant, size, images }) => {
  const canvasElRef = useRef(null);
  const [leftDrawerOpen,  setLeftDrawerOpen]  = useState(false);
  const [rightDrawerOpen, setRightDrawerOpen] = useState(false);
  ...
  // getAllDesignImages (same as before)
  const getAllDesignImages = useCallback(async () => {
    if (!canvasRef.current) return [];
    const fabric = canvasRef.current;
    const designImages = [];
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50 overflow-hidden">
      ...
      <header className="bg-white border-b border-gray-200 px-2 sm:px-4 py-2 flex items-center justify-between gap-2 shadow-sm flex-shrink-0">
        <div className="flex items-center gap-1 sm:gap-3">
          <h1 className="text-sm sm:text-lg md:text-xl font-semibold text-gray-800 truncate">T‑Shirt Studio</h1>
          <div className="h-4 w-px bg-gray-300 hidden sm:block" />
          <span className="text-[10px] sm:text-xs text-gray-500 hidden sm:inline">Design your own</span>
        </div>

        <div className="flex items-center gap-1 sm:gap-2">
          <button onClick={undo} disabled={!canUndo}
            className="px-2 sm:px-3 py-1.5 text-xs sm:text-sm bg-gray-100 hover:bg-gray-200 rounded-md disabled:opacity-40">
            ↩
          </button>
          <button onClick={redo} disabled={!canRedo}
            className="px-2 sm:px-3 py-1.5 text-xs sm:text-sm bg-gray-100 hover:bg-gray-200 rounded-md disabled:opacity-40">
          ...
          <div className="h-4 w-px bg-gray-300 hidden sm:block" />
          <button onClick={downloadImage}
            className="px-2 sm:px-3 py-1.5 text-xs sm:text-sm bg-gray-100 hover:bg-gray-200 rounded-md hidden sm:block">
            📸 Download
          </button>
          <button onClick={handleBuyNow}
            className="px-2 sm:px-3 py-1.5 text-xs sm:text-sm bg-green-600 hover:bg-green-700 text-white rounded-md">
            ⚡ Buy Now
          </button>
        </div>
      </header>

      {/* ── BODY (no overflow, completely visible) ──────────────────────── */}
      <div className="flex-1 flex overflow-hidden relative">

        {/* ── Mobile FABs (unchanged) ─────────────────────────────────────── */}
        <div className="lg:hidden fixed bottom-6 left-1/2 -translate-x-1/2 z-30
          flex gap-3 bg-white/90 backdrop-blur-sm rounded-full shadow-xl px-3 py-2 border border-gray-200">
          <button onClick={() => setLeftDrawerOpen(true)}
            className="flex flex-col items-center px-2 py-1 rounded-full active:bg-gray-100 touch-manipulation">
            <span className="text-base">🛠️</span>
            <span className="text-[9px] text-gray-600 font-medium">Tools</span>
          </button>
          <button onClick={() => setRightDrawerOpen(true)}
            className="flex flex-col items-center px-2 py-1 rounded-full active:bg-gray-100 touch-manipulation">
            <span className="text-base">⚙️</span>
            <span className="text-[9px] text-gray-600 font-medium">Actions</span>
          </button>
        </div>
        ...
        {/* ── CENTER CANVAS WITH T‑SHIRT BACKGROUND ──────────────────────── */}
        <main className="flex-1 flex flex-col items-center justify-center p-2 sm:p-4 bg-gradient-to-br from-gray-100 to-gray-200 overflow-hidden">

          {/* Zoom controls – smaller on mobile */}
          <div className="mb-2 flex items-center gap-1 sm:gap-2">
            <button onClick={ctx.zoomOut}
              className="w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-white shadow text-xs hover:bg-gray-50">－</button>
            <span className="text-[10px] sm:text-xs font-mono bg-white rounded-full px-2 py-0.5 shadow text-gray-600">
              {Math.round(ctx.zoom * 100)}%
            </span>
            <button onClick={ctx.zoomIn}
              className="w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-white shadow text-xs hover:bg-gray-50">＋</button>
          ...
          {/* T‑shirt container – responsive size, absolutely positioned canvas over the shirt */}
          <div className="relative inline-block">
            {/* Background t‑shirt image (replace with your own <img>) */}
            <div className="w-56 h-64 sm:w-64 sm:h-80 md:w-72 md:h-96 pointer-events-none">
              {/* Using the inline SVG as placeholder */}
              <TshirtBackground />
            </div>

            {/* Canvas overlay – smaller, centered over the t‑shirt */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-[45%] bg-white rounded shadow-inner p-1">
              <canvas
                ref={canvasElRef}
                className="block"
                style={{ maxWidth: '100%', width: '180px', height: '180px' }}
              />
            </div>
          </div>

          <p className="text-[10px] text-center text-gray-400 mt-1 sm:hidden">
            👉 Pinch &amp; zoom to adjust
          </p>

          {/* Page tabs – even smaller on mobile */}
          <div className="mt-2 flex items-center gap-1 flex-wrap justify-center">
            {ctx.pages.map(p => (
              <button key={p.id} onClick={() => ctx.setActivePage(p.id)}
                className={`px-2 py-0.5 rounded-full text-[10px] sm:text-xs font-semibold border transition-colors
                  ${ctx.activePage === p.id
                    ? 'bg-indigo-600 text-white border-indigo-600'
                    : 'bg-white text-gray-600 border-gray-300 hover:border-indigo-400'}`}>
                {p.label}
            ...
            <button onClick={ctx.addPage}
              className="px-2 py-0.5 rounded-full text-[10px] sm:text-xs font-semibold border border-dashed
                border-gray-300 text-gray-500 hover:bg-white">+ Page</button>
          </div>
        </main>
