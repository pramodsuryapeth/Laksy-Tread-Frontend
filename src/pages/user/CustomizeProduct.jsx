...
function CustomizeProduct() {
  ...
  // Derive variantImages and currentBgImage directly from selectedVariant (no effect needed)
  const variantImages = selectedVariant?.images || [];
  const currentBgImage = variantImages[currentIndex] || "/tshirt-mockup.png";
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-100 px-4">
        <div className="bg-white p-6 sm:p-8 rounded-lg shadow-md text-center w-full max-w-md">
          <h2 className="text-xl sm:text-2xl font-bold text-red-600 mb-4">Oops!</h2>
          <p className="text-gray-700 mb-6 text-sm sm:text-base">
            {error || "No product or variant data available."}
          </p>
          <button
            onClick={() => navigate("/products")}
            className="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded transition text-sm sm:text-base"
          >
            Back to Products
          </button>
        </div>
    ...
    <div className="min-h-screen bg-gray-100 py-4 sm:py-8 px-3 sm:px-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 mb-5 sm:mb-8">
          <button
            onClick={() => navigate("/")}
            className="flex items-center gap-2 text-gray-600 hover:text-gray-900 transition bg-white px-3 py-2 sm:px-4 sm:py-2 rounded-lg shadow-sm text-sm sm:text-base w-fit"
          >
            ← Back to Products
          </button>

          <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-800 text-center sm:text-left truncate">
            Customize {product.name}
          </h1>
        ...
        {/* Variant selector */}
        <div className="bg-white rounded-xl shadow-md p-3 sm:p-4 mb-4 sm:mb-6 flex flex-col sm:flex-row sm:flex-wrap sm:items-center gap-3 sm:gap-4">
          ...
          <select
            ...
            className="border border-gray-300 rounded-lg px-3 py-2 sm:px-4 sm:py-2 bg-white focus:ring-2 focus:ring-blue-500 text-sm sm:text-base w-full sm:w-auto"
          >
            {variants.map(v => (
              <option key={v._id} value={v._id}>
                {v.color} {v.stock && `(${v.stock} in stock)`}
          ...
          {selectedVariant.price && (
            <span className="sm:ml-auto text-base sm:text-lg font-bold text-green-600">
              ${selectedVariant.price}
            </span>
          )}
        </div>
        ...
        {variantImages.length > 1 && (
          <div className="bg-white rounded-xl shadow-md p-3 sm:p-4 mb-4 sm:mb-6">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={prevImage}
                className="bg-gray-200 hover:bg-gray-300 rounded-full p-1.5 sm:p-2 transition flex-shrink-0"
                aria-label="Previous view"
              >
                ◀
              </button>

              <div className="flex flex-1 overflow-x-auto gap-2 sm:gap-3 py-1 sm:py-2 scrollbar-hide">
                {variantImages.map((imgUrl, idx) => (
                  <button
                    key={idx}
                    onClick={() => handleImageChange(imgUrl, idx)}
